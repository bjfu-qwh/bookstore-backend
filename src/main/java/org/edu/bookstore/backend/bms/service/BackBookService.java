...
import org.edu.bookstore.backend.author.mapper.BackAuthorWorkMapper;
import org.edu.bookstore.backend.bms.dto.NewBookDTO;
...
import org.edu.bookstore.backend.category.mapper.CategoryBookMapper;
...
import org.edu.bookstore.backend.util.PagedDTOUtil;
...
public class BackBookService {
    ...
    public JSONResult<PagedDTO<BookVO>> getBookTable(Integer pageID, Integer pageSize) {
        Page<BookVO> page = new Page<>(pageID, pageSize);
        List<BookVO> data = bookMapper.getBookVOPage(page);
        return JSONResultUtil.successWithDataOnly(PagedDTOUtil.create(
                page.getTotal(), pageID, pageSize, data
        ));
    }
